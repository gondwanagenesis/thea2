...
## Behavior spec
- **Delivery is at-least-once with ledger dedupe.** The poll loop: fetch batch → for each update, `recordInbound` (returns false on duplicate) → if new: enqueue to the pipeline, THEN commit the offset. A crash anywhere before offset commit ⇒ redelivery on restart ⇒ `recordInbound` returns false ⇒ dropped as duplicate. No message is lost by the ordering; none is handled twice (the S2 crash-replay test is exactly this sequence with the process killed between handle and commit).
