...
module: M21
name: spine
syncedTo: W2 as-built (2026-09-03 — P-SPINE-1 landed; P-LOOP wiring landed the same day: compose constructs OpenCodeRunner on the prod branch when cfg.spine is set, the loop's assess rides the seam, the M.6 config block is live)
stage: W2
...
`test/spine/` — `fake-runner.test.ts` (`fake-runner-replays-golden-turn`, `native-loop-and-fake-runner-agree-on-decide-shape`), `runner.test.ts` (`runner-supervises-and-restarts-a-dead-spine` incl. wedge→abandon, `session-break-forks-a-new-session`, `sse-events-map-to-l0-model-call-events`, POST body shape), `decide.test.ts` (`decide-arrives-as-validated-object-or-repairs-once`, `a string bubbles field becomes a one-element array`, format-on-first-POST, failure-path), `packet.test.ts` (`packet-render-golden-unchanged-through-spine`, `inhibition-is-the-trailing-message`, [PROCEDURAL] placement), `gates.test.ts` (`gate-veto-blocks-a-tool-call-in-replay`, `every-veto-emits-a-gate-event`, permission compile, fail-open), `config.test.ts` (M.6 block). The LOOP side of the seam proves in `test/loop/spine-seam.test.ts` (P-LOOP: decide through the normal decision path, tool defs + contract across the seam, streamed tool round mediation, text-delta prose fold, native regression). Hermeticity: every runner test runs against a local `node:http` stub on 127.0.0.1 (ephemeral port) speaking the documented v1.18.x shape from recorded fixtures (`test/spine/fixtures/`), with the spawn seam injected so the real `opencode` binary is never launched, downloaded, or required (D.7-3). 27 tests, all green.

## As built (W2, P-SPINE-1 + P-LOOP)
- **Compose wiring LANDED (P-LOOP, 2026-09-03 — was held for the coordinator at P-SPINE-1).** `compose` resolves the spine config on the prod branch only (`preset === 'prod' && cfg.spine !== undefined`) via `resolveSpineConfig(cfg.spine, opts.env ?? process.env)` with her door passed in from the wiring site (`model: {providerID:'voice', modelID: cfg.models.doors.voice.model, door:'voice'}` — never pinned in the yaml), constructs `OpenCodeRunner`, and hands it to `makePipeline({runner})` → `LoopDeps.runner` → `TurnState.runner`. A bad block or a missing token is a STARTUP failure (resolveSpineConfig throws at compose). `System.stop()` drains the pipeline THEN stops the runner (G3: no orphaned spine child). Hermetic/probe presets never construct a runner, and no test launches the binary (D.7-3).
- **The M.6 config block is LIVE** (`thea2.config.yaml`: version 1.18.3, port 4096, authTokenEnv THEA2_SPINE_TOKEN; opencode + bun provisioned on the VPS, the token in /etc/thea2/keys.env). Deleting the block returns her to the native loop (rule 5). `loadConfig`'s secret scanner gained a principled exemption: `*Env`-suffixed keys carry env VARIABLE NAMES by config law (`keyEnv`, `authTokenEnv`), so the key-name suspicion does not fire on them — their values are still scanned by `secretShaped`, so real credential material under a `*Env` key is still rejected.
- **The loop-side half of the turn-abort contract is the native failure path (P-LOOP):** a spine turn ending `stop-reason:'error'` folds into an empty ChatResponse, the loop's repair rung / failure-silence machinery handles it as any dead reply — `incident.spine_failed` (runner side) + the forced-silent lock (loop side), the reply stays owed.
