...
module: M08
name: derive
syncedTo: v6-W2 as-built (2026-09-03 — P-JUDGE0 JU.1/JU.2: judge hard-fail laws, dash normalization before grading)
stage: v6-W2
...
- **Judge**: pinned rubric text (`JUDGE_SYSTEM_PROMPT`), structured verdict via the model ladder's `emit` tool (rung b), `taskClass 'judge'` / `tier 'reasoning'` / `temperature 0`. Constants `JUDGE_VERSION = 'derive-judge-v2'`, `JUDGE_PASS_THRESHOLD = 4`. The rubric **prepends the hard-fail laws** (v6 JU.1): a draft carrying a romantic pet name, a fabricated dwelling/home/pet/named third party, physical co-presence or touch, an invented past event, or an em-dash scores 1 regardless of how well it is written. Retry policy: exactly 2 attempts per target (`rng.fork(deriveKey + '::attempt-N')`), then the draft is discarded — no file, no manifest entry; `failures[]` records each attempt's stage (`generate` | `parse` | `judge`) and code (`derive/draft-shape` when the threshold fails).
- **Dash normalization** (v6 JU.2): before anything downstream sees a generated draft, `run.ts` applies `normalizeDashes` (`src/derive/judge.ts`) — em-dash (—) and en-dash (–) become a plain hyphen. Parse validation, judging, and the accepted bytes all use the same normalized text: what was judged is what ships. The judge prompt still carries the em-dash law as the backstop for any caller that skips the pipeline.
...
### As built — v6 W2, P-JUDGE0 JU.1/JU.2 (2026-09-03)

- **JU.1 — the judge knows the laws.** `JUDGE_SYSTEM_PROMPT` prepends a `HARD FAIL LAWS` block ahead of the score scale: score 1 on ANY of a romantic pet name; a fabricated dwelling, home, pet, or named third party; physical co-presence or touch; an invented past event; an em-dash or en-dash. `JUDGE_VERSION` bumped `derive-judge-v1` → `derive-judge-v2` (`src/derive/types.ts`) so manifest entries record which rubric attested them. Named tests in `test/derive/judge.test.ts`: `a girlfriend-line variant fails`, `a variant that adds a lamp fails`, `a faithful variant passes` (plus the law-prompt pin and the em-dash backstop at the judge itself).
- **JU.2 — normalization + angles.** `normalizeDashes` (— / – → `-`) is applied in `run.ts` to the whole draft text before parse, judge, and write. The mood-variant `ANGLES` lost "one concrete physical detail" (the generator must not ask for what the judge fails), the sideways angle's em-dash became a colon, and the rendered context separator is ` - ` instead of `—` (a plain-safe YAML scalar now — no quoting). Named run-level test: `em-dash-normalized-before-judging` (`test/derive/run.test.ts`).
- **Rubric share (M10).** `src/consolidate/draft.ts` carries the same law list in its judge prompt, worded against episodes, with its own `JUDGE_VERSION` bumped to `consolidate-judge-v2`. The text is deliberately duplicated, not imported: `src/derive` is not an allowed edge for M10.
- **Regeneration owed (D.6-7, M.4).** The committed `corpus/derived/**` was judged under `derive-judge-v1` and its mood-variant files carry the old em-dash context separator. `corpus:check` stays green on them (it checks `judge.pass`, not the rubric version), so the obligation is a quality one: delete the loaded derived set and regenerate on the dev box with the v2 judge before the W2 landing commit. ANGLES and the context separator are not part of `templateHash`, so no deriveKey dirties on its own — the regeneration is explicit, not automatic.
