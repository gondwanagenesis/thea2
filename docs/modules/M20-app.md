...
## As built (P-LOOP spine wiring, v7-W2, 2026-09-03)

- **`PipelineDeps.runner?: SpineRunner` (M21/P-LOOP).** compose constructs the `OpenCodeRunner` on the prod branch only (`preset === 'prod' && cfg.spine !== undefined`), resolving the spine config with her voice door as the model (`resolveSpineConfig({...cfg.spine, model: {providerID:'voice', modelID: cfg.models.doors.voice.model, door:'voice'}}, opts.env ?? process.env)`); hermetic/probe presets and an absent spine block keep the native loop (rule 5). A bad block or a missing `spine.authTokenEnv` token is a STARTUP failure — compose names the stage by throwing at boot. `makePipeline` threads the runner into every `LoopDeps`; `System.stop()` drains the pipeline (in-flight turns may still be mid-POST) and THEN stops the runner — thead never orphans the spine child (G3). `ComposeOpts.env` is the injection seam for the token (main owns the process edge; compose already sits at the process boundary for cwd/stderr). Tests: `test/app/config.test.ts` (`spine block (M.6) and the *Env secret-scanner exemption` describe), seam behavior in `test/loop/spine-seam.test.ts`.
- **Secret scanner `*Env` exemption (config.ts).** `findSecrets` no longer treats a `/token|secret|.../i`-named key whose name ends in `Env` as a secret hit — those keys carry env VARIABLE NAMES by config law (`doors.*.keyEnv`, `spine.authTokenEnv`), and the old rule made the M.6 block unloadable. Values under `*Env` keys are still scanned by `secretShaped`, so credential material pasted there is still rejected (`app/config-secret-in-yaml`).
