...
import { EXPECTED_INBOUND, fixture, fixtureNames, parseFixture, testSpeaker } from './helpers.js';
...
describe('FakeChannel ⇔ real parse layer conformance (recorded getUpdates fixtures)', () => {
  ...
  it('both sides agree on exactly which fixtures reach the pipeline', async () => {
    ...
    expect(canonicalJson(got)).toBe(canonicalJson(expectedOrder.map((n) => EXPECTED_INBOUND[n]!)));
  });
});
