...
8. **Corpus discipline.** `corpus/canon/` is human-edited — agents never write it (only `scripts/` tooling and the human). `corpus/derived/` is written only by M08's pipeline. `corpus/lived/` only by M10's consolidators. `corpus/proposals/` only by M10, merged only by the human.
