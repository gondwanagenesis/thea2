...
// Constants are v6 CA.1 (SNAPPY AND TRUE): bubble 1 leaves at the lock
// instant, and the whole delivery lands inside 12 s.
...
// --- the spec's constants, verbatim (AGENTS.md §6: these are load-bearing; v6 CA.1) ---
export const PRE_DELAY_BASE_MS = 0; // bubble 1's timeline starts at the lock instant
export const PRE_DELAY_PER_RELUCTANCE_MS = 1500; // applied ONLY at/above RELUCTANCE_DELAY_GATE
export const RELUCTANCE_DELAY_GATE = 0.5; // below this reluctance carries no pre-delay at all
export const CPS_FLOOR = 25; // typing speed at the arousal rail a=-1
export const CPS_CEIL = 45; // typing speed at the arousal rail a=+1
export const LOW_VALENCE_CPS_FACTOR = 0.85; // −15% typing speed when a[valence] < 0
export const GAP_MIN_MS = 400;
export const GAP_MAX_MS = 1500;
export const GAP_JITTER = 0.15; // ±15% around the arousal curve — the only rng draw in the module
export const TOTAL_CAP_MS = 12_000;
...
/**
 * The pause before bubble 1: zero unless reluctance reached the gate — a reply
 * she is not reluctant about starts the instant the decision locks (v6 CA.3);
 * at or above the gate the delay grows linearly with the reluctance.
 */
export const preDelayMs = (reluctance: number): number =>
  reluctance < RELUCTANCE_DELAY_GATE
    ? PRE_DELAY_BASE_MS
    : PRE_DELAY_BASE_MS + PRE_DELAY_PER_RELUCTANCE_MS * reluctance;

/** Typing speed in chars/second: the 25→45 lerp over the arousal deviation, slowed 15% under low valence. */
...
/** Inter-bubble gap: 1500→400 ms shrinking with arousal, ±15% jitter from the forked stream, clamped to the envelope. */
...
export const planDelivery = (
  ...
  const steps: DeliveryStep[] = [{ kind: 'pause', ms: msOf(preDelayMs(reluctance)) }];
...
/**
 * The 12 s ceiling: past it, every pause and typing duration scales down by the
